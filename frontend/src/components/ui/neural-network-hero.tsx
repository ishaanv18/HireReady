...
interface HeroProps {
    ...
    badgeText?: string;
    badgeLabel?: string;
    ctaButtons?: Array<{ text: string; href: string; primary?: boolean; onClick?: (e: React.MouseEvent<HTMLAnchorElement>) => void }>;
...
export default function NeuralNetworkHero({
    ...
    microDetails = ["AI Interview Practice", "Resume Analysis", "Real-time Feedback"]
}: HeroProps) {
        ...
        <section ref={sectionRef} className="relative h-screen w-screen overflow-hidden">
            ...
            <div className="relative mx-auto flex max-w-7xl flex-col items-start gap-6 px-6 pb-24 pt-36 sm:gap-8 sm:pt-44 md:px-10 lg:px-16">
                <div ref={badgeRef} className="inline-flex items-center gap-2 rounded-full border border-purple-300/20 bg-purple-500/10 px-3 py-1.5 backdrop-blur-sm">
                    <span className="text-[10px] font-light uppercase tracking-[0.08em] text-purple-200">{badgeLabel}</span>
                    <span className="h-1 w-1 rounded-full bg-purple-300/60" />
                    <span className="text-xs font-light tracking-tight text-purple-100">{badgeText}</span>
                </div>
                ...
                <div ref={ctaRef} className="flex flex-wrap items-center gap-3 pt-2">
                    {ctaButtons.map((button, index) => (
                        <a
                            ...
                            className={`rounded-2xl border border-purple-300/20 px-5 py-3 text-sm font-light tracking-tight transition-colors focus:outline-none focus:ring-2 focus:ring-purple-400/50 duration-300 ${button.primary
                                ? "bg-purple-500/20 text-white backdrop-blur-sm hover:bg-purple-500/30"
                ...
                <ul ref={microRef} className="mt-8 flex flex-wrap gap-6 text-xs font-extralight tracking-tight text-purple-200/70">
                    {microDetails.map((detail, index) => {
                        const refMap = [microItem1Ref, microItem2Ref, microItem3Ref];
                        return (
                            <li key={index} ref={refMap[index]} className="flex items-center gap-2">
                                <span className="h-1 w-1 rounded-full bg-purple-300/60" /> {detail}
                            </li>
                        );
                    })}
                </ul>
